...
SEARCH_URL = f"{BASE_URL}/gulp2/search/project"

# TODO: validate selectors against live site markup
SEL_LISTING = "div.project-item, article.project, .search-result-item"
SEL_TITLE = "h2 a, h3 a, .project-title a, .title a"
SEL_COMPANY = ".company, .client"
SEL_LOCATION = ".location, .city"
SEL_DATE = ".date, time, .published"
SEL_RATE = ".rate, .budget"
SEL_DESCRIPTION = ".description, .excerpt, .abstract"


class GulpScraper(BaseScraper):
    """Scraper for gulp.de project listings."""

    site_name = "gulp.de"

    def search(self, keywords: List[str], locations: List[str]) -> List[JobPosting]:
        if not self._is_allowed_by_robots(BASE_URL, "/gulp2/search/project"):
        ...
        for keyword in keywords:
            ...
            params = {"keyword": keyword, "location": "", "distance": ""}
